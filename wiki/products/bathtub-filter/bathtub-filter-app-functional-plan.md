...
- ✅ **参比色卡进包装 BOM** —— **D-16 已裁定（2026-09-06）**。⚠️ 但**工艺约束严格**（哑光禁覆亮膜 / 中性灰+白块 / DPD 色阶 / **批次码** / 防潮 / 可同框），做错任一条卡片照印但读数不可用且不报错——规格见 [决策台账 §参比色卡规格要求](./bathtub-filter-decision-register.md)
- 🔴 **试纸量程选型** → 挂 **D-11**（与全链路审查 **F6** 同源，应合并决策）；进水场景需覆盖 0.5–4 ppm，与出水验证场景量程要求不同 —— **这是 L2 剩余的唯一硬阻塞**
...
| 期 | 范围 | 载体 | 依赖 |
|---|---|---|---|
| **MVP**（V1 同期） | L0 + L1 + **L2 时机①（进水）** + L3 可解释寿命 | **Web PWA** | ✅ 参比色卡（D-16 已裁定）；🔴 试纸量程（D-11 + F6）**仍阻塞** |
...
| # | 决策 | 影响 | 建议挂靠 |
|---|---|---|---|
| ~~1~~ | ~~参比色卡进包装 BOM~~ | ✅ **已裁定 2026-09-06** —— 转为供应链执行项（打样 + 与目标机型实拍验证 + 与 D-15 合并刀模） | **D-16** |
| 2 | 试纸量程（进水 0.5–4 ppm vs 出水低量程，是否两种） | L2 准确度 + COGS；**MVP 唯一硬阻塞** | **D-11 + F6** |
