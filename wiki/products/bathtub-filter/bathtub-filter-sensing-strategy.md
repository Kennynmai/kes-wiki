...
L2 的管线是：**拍照 → 目标定位 → 对已知参照物标定 → 判读 → 置信判断 / 拒答**。换掉判读对象即可服务另外两个场景：

| 场景 | 判读对象 | 状态 |
|---|---|---|
| **L2 试纸读数** | 色垫（色彩恒常） | ✅ 参比色卡已进 BOM（D-16），随 V1 出货 |
| 安装环境识别 | spout 形态 → [S-01~S-08 归类](./bathtub-filter-supported-spout-matrix.md) | 🟡 增量应用 |
| 替换地漏盖识别 | 直径 / 螺孔距（几何） | 🟡 见 [[us-eu-home-hardware-mass-customization-2026]] §9.4 |

**共用的是拍摄 UX 与拒答逻辑——这是两条线共同的头号难点**，与判读物理无关。**不共用的是判读物理本身**：试纸读准了不证明螺孔距能读准。

**排序含义**：L2 先发且边际成本近零（色卡已进 BOM、随货出、有 DPD 真值），**应由它先验证"拍摄 UX + 拒答"这一半风险**，另两个场景复用其成果，不并行造两套。详见 [[us-eu-home-hardware-mass-customization-2026]] §9.6。

**安装环境识别的诚实边界**：[评论与退货风险页](./bathtub-filter-review-patterns-and-return-risk.md)指出「universal fit」语言非常危险、带来可避免的退货，**购前拍照核验是"通用声称"的替代品**——用识别换掉一个不能说的 claim。但适配**不是**第一退货动因（整体无效 46.5% > 质量 19.5% > 绕流/溢流 19.0%）→ **不足以单独立项**，作 L2 跑通后的增量评估。
