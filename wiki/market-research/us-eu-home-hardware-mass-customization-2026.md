...
### 9.6 与浴缸滤芯感知线的共用原语（2026-09-06 追加）

§9.5 #2 的卡点（**拍照识别需参照物校准，未做原型**）与 [[bathtub-filter-sensing-strategy]] L2 的手机读试纸**共用同一条管线**：

```
拍照 → 目标定位 → 【对已知参照物标定】 → 判读 → 置信判断 / 拒答
```

差别只在判读对象：一个是**色垫**（色彩恒常），一个是**尺寸/螺孔距**（几何测量）。

**可迁移的（且是主要风险所在）**

| 项 | 说明 |
|---|---|
| **拍摄 UX** | 让普通用户拍出"参照物同框、光照可用、角度可校正"的照片——**这是两条线共同的头号难点**，与判读物理无关 |
| **参照物同框强制 + 坏图拒答** | 宁可不给读数，不给错读数 |
| 后端标定档 / 批次码架构 | 参照物换批次时如何追溯校正 |

**不可迁移的**：判读物理本身。色彩恒常与几何测量是两个问题，**试纸读准了不证明螺孔距能读准**（§9.5 #2 的 1/4" 判别仍需独立验证）。

**排序含义（这是本节的实际结论）**

浴缸滤芯 L2 **先发且边际成本近零**——参比色卡已于 2026-09-06 裁定进包装 BOM（[[bathtub-filter-decision-register]] D-16），随 V1 出货，且有 DPD 实验室值可作真值。因此：

> **让 L2 先跑，用它验证"拍摄 UX + 拒答逻辑"这一半风险；§9.4 的 AI 识别助手复用其成果，不并行造两套。**
> L2 若在拍摄 UX 上就失败，等于对 §9.4 切入点 #1 的提前 kill signal——**这是一次很便宜的预警**。
...
- [[bathtub-filter-sensing-strategy]] / [[bathtub-filter-app-functional-plan]] — **§9.6 的共用相机管线**：拍照 → 参照物标定 → 判读；浴缸滤芯 L2 是更早、更便宜的验证载体
