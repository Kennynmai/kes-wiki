...
3. **活性炭测试日志（非当前 V1）**：coconut-shell vs 其他 carbon 的选型 + acid-wash 后的 commissioning 验证（2024-07–2024-08）；该资料仅适用于其他含碳版本，不纳入当前 V1 卖点
4. **流速/溢水/过滤棉分层测试**（2024-11-07）：确定 top stack 的 overflow envelope
5. **导流模块 vs 无 的测试**（2025-10-22）：确认 CaSO₃ 层需要导流以避免中心冲蚀
6. **产品爆炸图 / 当前 V1 口径**：strap handle → 水箱外壳 → PP棉物理过滤 → KDF55 110g（金色介质）→ CaSO3 130g（白色颗粒）
...
category default 的 mixed bead bed 有四个结构性问题：**channeling / 介质互相干扰 / 无法分段更换 / 无法诊断失效模式**。KES 选择严格 PP cotton / KDF55 / CaSO3 分层 + inner 导流模块 + 顶部溢流槽，强制水流**穿过**每一层而不是沿阻力最小路径绕开。
...
**关键结论**：bath-fill 场景的 EBCT 对 KDF 来说太短，无法让 KDF 做主力去游离氯。**CaSO₃ 是主力去游离氯 KPI；KDF 是安心层 + 寿命稳定层**。链式公式：系统总游离氯去除 = 1 − (1 − KDF) × (1 − CaSO₃)。
...
**Replacement 触发锚**：
- soft trigger：系统总游离氯去除跌到 90%（体验仍好）
- strong trigger：跌到 80%（进入快速衰减）
- mandatory：跌到 50%（视为失效）

### 6）流速 / 溢水 envelope（内部实测，Doc 4；非 V1 滤材条件）
测试条件：204 g KDF（10–40 目）+ 45 g acid-washed carbon（8–10 目），0.5 MPa，瞬时 38–40 L/min，目标 178 L。该测试不是当前 V1 的 CaSO3 130g + KDF55 110g + PP棉配置，只能作为结构/溢水 envelope 的历史参考，不能作为 V1 含碳、no-pre-rinse 或 no-carbon-dust 卖点依据。
...
### 7）活性碳选型（Doc 3；非当前 V1）
当前 V1 不含活性炭；本段仅保留为其他含碳版本资料，不能纳入 V1 卖点。
...
- **V1 口径**：不写 "no pre-rinse required"、"no carbon dust"、"acid-washed coconut-shell carbon"
...
- carbon 不冲粉属于其他含碳版本资料，不纳入当前 V1 卖点
