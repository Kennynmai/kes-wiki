...
配置：35 孔 PP 棉盘 15 mm + KDF55 110 g + CaSO₃ 130 g（⚠️ **2026-09-05 注：这是裁定前配置。V1 出货配置为 PET 盘 + KDF55 130 g + CaSO₃ 110 g，主力介质少 15%，本组数据只作方向参考，幅度不可沿用**）；原水 **5 mg/L 自由氯**（≈2.5× 寿命模型 2 ppm 基线）；氯试纸比色法。
...
固定基准配置（生产规格，2026-09-05 按 [V1 BOM 表](./bathtub-filter-v1-free-chlorine-removal-dimensions-materials.md) 更正）：PET 盘 10 mm / 120 g/m² + **KDF55 130 g + CaSO₃ 110 g**，**3–4 mm 生产粒径**，水温 **40±2℃**，新芯（throughput=0）。
...
| CaSO₃ 质量 | 90 g | **110 g（V1 基准）** | 130 g（旧配置回归对照）| 160 g |
|---|---|---|---|---|
| 去氯率 | ☐ | ☐ | ☐ | ☐ |
| 床高/截面 | ☐ | ☐ | ☐ | ☐ |
| 压降 ΔP | ☐ | ☐ | ☐ | ☐ |
...
| 累计通水量 | 0 L（新芯）| 1,000 L | 2,000 L | 2,945 L（≥99% 段末模型值，110 g 口径）| 寿命口径终点 |
|---|---|---|---|---|---|
| 去氯率 | ☐ | ☐ | ☐ | ☐ | ☐ |

输出：实测寿命衰减曲线，验证/证伪 η=0.9 线性模型与「mandatory ≈121 baths（2 ppm）」口径。
...
### 2.5 装填与溢流复核（2026-09-05 新增，Gate 1 送测前先做）

- **KDF 仓装填高度实测**：按 BOM 几何（内径 117 mm、中心柱 Ø50 mm、有效高 14 mm）核算，130 g KDF55 只占床容积约 36–44%，摊平 5–6 mm。送测前记录实际装填高度、是否有压料结构、25 L/min 下床层是否位移。若确为半空，改壳或加隔板后再测。
- **V1 配置溢流包络复测**：现有 35 L/min 无溢流数据来自 204 g KDF + 45 g 炭 + 两层纤维盘配置。用 V1（单片 PET 盘 + 130/110）在 20 / 25 / 30 / 35 L/min 复测，记录溢流起点。
- **泡泡浴 / bath bomb 通过浴盐仓**：0.7 mm × 3,846 孔板下的起泡与顶溢观察，一次即可。
...
> 阈值里的「≥85%」基线来自 2026-03-20 在 5 ppm 高压力下 27L/min 仍达 84%（**该测试为 CaSO₃ 130 g 旧配置，V1 为 110 g，阈值在 Gate 1 前视为待校准**）；真实 1–2 ppm 下应不低于此，故 85% 是合理且偏保守的及格线。**若第三方实测显著低于此，说明 5 ppm 比色单测高估，正是必须早发现的事。**
