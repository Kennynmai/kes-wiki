...
> ⚠️ **2026-07-02 裁定注（按 [V1 BOM 表](./bathtub-filter-v1-free-chlorine-removal-dimensions-materials.md) 更正，§9 已重算（2026-07-02）；§9.5 bench test 仍按旧配比记述）**：产品已裁定滤料克数按 BOM 互换——**KDF55 = 130g、CaSO₃ = 110g**（本页正文仍按旧配比 KDF55 110g / CaSO₃ 130g 记述）。连锁影响：§9 寿命模型的质量比缩放应由 3.25×（130/40）改为 **2.75×（110/40）**，模型容量应重算为 **~21,550 L @2ppm**（对应 ~50,934 L @1ppm → **~43,100 L @1ppm**）；§9.5 的 KDF 贡献率 43/32/26/21% 系**旧配比（KDF55 110g）实测**，新 130g KDF 配比需复测。证据等级不变（🟡 供应商内部模型 / 待 Gate 1 DPD）。
...
### 基础参数
- CaSO₃ 质量：**110 g**（3–4 mm 颗粒）
- 效率系数：**η = 0.9**（放大自"宁立"40g / 0.5–1mm / 8 L/min / 2 ppm 自由氯 参考报告，按质量比 2.75 × 0.9 缩放）
- 水温：38–42°C
- 泡澡基准：70 gal 满缸，2/3 填充 = 47 gal = **178 L/次**；3 次/周 = 12 次/月
- KDF 贡献系数：见 [[bathtub-filter-technology-notes]] EBCT 表
- 系统总游离氯去除：**1 − (1 − KDF) × (1 − CaSO₃)**

### 2 ppm 自由氯条件（US 典型偏高段）— "系统总游离氯去除" 寿命表

| 阶段 | 累计水量 | 泡澡次数（178 L/次）| 周数（3 次/周）|
|---|---|---|---|
| 系统 ≥99%（最佳体验段） | ~2,945 L | 16.5 | 5.5 |
| 系统 ≥95% | ~14,953 L | 84.0 | 28.0 |
| 系统 ≥90%（软换触发） | ~17,028 L | 95.7 | 31.9 |
| 系统 ≥80%（强换触发） | ~21,083 L | 118.4 | 39.5 |
| 系统 <50%（强制换） | ~21,550 L | 121.1 | 40.4（约 9–10 个月）|

### 1 ppm 自由氯条件（US 典型常见段）— 寿命约 ×2

| 阶段 | 累计水量 | 泡澡次数 | 周数 |
|---|---|---|---|
| 系统 ≥99% | ~5,891 L | 33.1 | 11.0 |
| 系统 ≥95% | ~29,906 L | 168.0 | 56.0 |
| 系统 ≥90% | ~34,056 L | 191.3 | 63.8 |
| 系统 <50%（强制换） | ~43,100 L | 242.1 | 80.7（约 1.5 年）|

### 替换触发语义（非月数口径）
- **Soft trigger**：系统总游离氯去除跌至 90% → 体验仍好，提示用户开始准备替换
...
3. 可直接用于订阅节奏设计：99% 段约 16.5 次浴 ≈ 5.5 周 → 小批量早期订阅；90%/80% 段约 96/118 次浴 ≈ 强制换约 9–10 个月一次常规订阅
...
供应商宗立 2025-12-26 报告 ZONET20251113001（40g / 0.5–1mm / 8 L/min / 2.0±0.2 mg/L 自由氯）的 4 个插值档 × 2.475 后，全部与 Version A 表内数字吻合：
- ≥99%: 1,190 L × 2.475 = **2,945 L** ✅
- ≥95%: 6,042 L × 2.475 = **14,954 L** ✅
- ≥90%: 6,880 L × 2.475 = **17,028 L** ✅
- ≥50%: 8,707 L × 2.475 = **21,550 L** ✅
