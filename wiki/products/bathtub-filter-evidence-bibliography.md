---
type: product
status: draft
owner: strategy
created: 2026-04-11
updated: 2026-04-11
visibility: team
confidence: medium
officiality: draft
domain: product
domains: [bathtub-filter, evidence, bibliography]
source_count: 6
review_cycle: monthly
verification_status: spot-checked
related:
  - ./bathtub-filter.md
  - ./bathtub-filter-claims-and-certifications.md
  - ./bathtub-filter-category-boundary.md
---

# Bathtub Filter Evidence Bibliography

## Why this page exists
This page indexes the evidence base behind bathtub-filter category logic.

Its purpose is to separate:
1. **problem-level evidence** — why consumers care about chlorine, hard water, bathing, and skin comfort
2. **product-level evidence** — whether bathtub filters themselves are proven effective

At present, the evidence base is much stronger for **problem relevance** than for **finished-product efficacy**.

## Evidence-use rule
Use this page to support:
- category relevance
- consumer problem framing
- segment plausibility
- claim-discipline and caution

Do **not** use this page alone to support:
- finished-product efficacy claims
- disease-improvement claims
- strong hard-water-softening promises for bathtub filters

---

## A. Academic / peer-reviewed evidence

### 1. Free residual chlorine in bathing water reduces the water-holding capacity of the stratum corneum in atopic skin
- Source type: PubMed-indexed article
- URL: `https://pubmed.ncbi.nlm.nih.gov/12692355/`
- Access level obtained in this session: **long abstract extracted from page HTML**
- What it supports:
  - chlorine in bathing-water contexts can be relevant to skin-barrier concerns in atopic skin
- What it does **not** support:
  - that a given bathtub filter product improves eczema or skin outcomes
- Evidence role: **problem-level support**

### 2. Association between domestic water hardness, chlorine, and atopic dermatitis risk in early life
- Source type: PubMed-indexed population-based cross-sectional study
- URL: `https://pubmed.ncbi.nlm.nih.gov/27241890/`
- Access level obtained in this session: **accepted manuscript / near-full text obtained**
- Better-access locations identified:
  - KCL Pure record: `https://kclpure.kcl.ac.uk/portal/en/publications/the-association-between-domestic-water-hardness-chlorine-and-atop/`
  - DocsLib accepted-manuscript mirror: `https://docslib.org/doc/4417330/the-association-between-domestic-water-hardness-chlorine-and-atopic-dermatitis-risk-in-early-life-a-population-based-cross-sectional-study`
- Visible abstract signal:
  - higher domestic water CaCO3 associated with increased AD risk in infancy
  - chlorine influence remained uncertain in the visible summary
- What it supports:
  - hard water is a serious category-relevant variable
- What it does **not** support:
  - direct bathtub-filter efficacy
- Evidence role: **problem-level support**

### 3. The effect of water hardness on atopic eczema, skin barrier function: A systematic review, meta-analysis
- Source type: PubMed-indexed systematic review / meta-analysis
- URL: `https://pubmed.ncbi.nlm.nih.gov/33259122/`
- Access level obtained in this session: **detailed repository abstract obtained**
- Better-access location identified:
  - KCL Pure record: `https://kclpure.kcl.ac.uk/portal/en/publications/the-effect-of-water-hardness-on-atopic-eczema-skin-barrier-functi/`
- Visible abstract signal:
  - hard water associated with AE in children
  - no evidence that domestic water softeners improve objective severity in established AE
- What it supports:
  - hard water matters in category logic
  - claim discipline is necessary
- What it does **not** support:
  - “soften water and eczema improves” product storytelling
- Evidence role: **high-value caution source**

### 4. Atopic eczema and domestic water hardness
- Source type: PubMed-indexed article
- URL: `https://pubmed.ncbi.nlm.nih.gov/9716057/`
- What it supports:
  - hard water / eczema linkage has longstanding research history
- Evidence role: **background support**

### 5. Interactions between domestic water hardness, infant swimming and atopy in the development of childhood eczema
- Source type: PubMed-indexed article
- URL: `https://pubmed.ncbi.nlm.nih.gov/22591883/`
- What it supports:
  - interaction between hard water, infant swimming, and atopic status in childhood eczema discussion
- Evidence role: **baby / infant segment support**

### 6. The association between domestic hard water and eczema in adults from the UK Biobank cohort study
- Source type: PMC / cohort study
- URL: `https://pmc.ncbi.nlm.nih.gov/articles/PMC9804584/`
- What it supports:
  - hard-water / eczema discussion extends beyond infant-only context
- Evidence role: **adult-context support**

---

## B. Review / overview evidence

### 7. Pooling the evidence: A review of swimming and atopic dermatitis
- Source type: PMC review article
- URL: `https://pmc.ncbi.nlm.nih.gov/articles/PMC10946598/`
- Access level obtained in this session: **title + snippet only; direct article-body retrieval blocked by browser check / reCAPTCHA**
- What it supports:
  - chlorine exposure can be irritating in some contexts
  - literature remains nuanced
- Evidence role: **contextual support / nuance**

### 8. Effects of Water Bathing on Atopic Dermatitis Are Determined by the Constituents in the Water
- Source type: Wiley / Dermatologic Therapy article
- URL: `https://onlinelibrary.wiley.com/doi/full/10.1155/dth/3695790`
- Current access status:
  - only indexed/visible citation trail and public snippet level captured so far
- Evidence role: **to review more deeply if access improves**

---

## C. Institutional / authority-style references

### 9. EPA — CCR Information for Consumers
- Source type: US EPA
- URL: `https://www.epa.gov/ccr/ccr-information-consumers`
- Access level obtained in this session: **body text / page content partially extracted from public HTML**
- What it supports:
  - consumers can check local annual water quality reports
  - local-water education can be grounded in official sources
- Evidence role: **official context / educational support**

### 10. CDC — Chloramines and Pool Operation
- Source type: CDC Healthy Swimming
- URL: `https://www.cdc.gov/healthy-swimming/toolkit/chloramines-and-pool-operation.html`
- What it supports:
  - chloramines can irritate skin, eyes, and respiratory tract in pool settings
- Important limit:
  - pool context should not be overextended directly to household bathtub claims
- Evidence role: **official irritation-context support**

### 11. National Eczema Society — Swimming and eczema
- Source type: condition-specific institution / society guidance
- URL: `https://eczema.org/information-and-advice/triggers-for-eczema/swimming-and-eczema/`
- Access level obtained in this session: **meaningful body paragraphs extracted from public HTML**
- What it supports:
  - chlorine can contribute to dryness / irritation
  - chlorine is not straightforwardly “all bad” in every eczema context
- Evidence role: **high-value nuance / anti-overclaim reference**

---

## D. Market / adjacent-industry evidence

### 12. Shower-filter market report summaries (multiple vendors)
- Types observed:
  - Verified Market Research
  - DataHorizzon Research
  - Business Research Insights
  - Market Research Intellect
  - related market-report pages
- What they support:
  - chlorine concern, hard water concern, skin/hair concerns, and wellness framing are commercially recognized adjacent drivers
- What they do **not** support:
  - standalone bathtub-filter TAM
- Evidence role: **adjacent-market support**

---

## Evidence strength summary
### Strongest support currently available
- hard water as a real eczema-related problem variable
- chlorine / bathing-water sensitivity as a plausible skin-barrier concern
- bathtub filter category logic as a credible consumer concern area

### Weakest / missing areas
- product-level finished bathtub-filter efficacy evidence
- direct trial-level support for bathtub filters improving consumer skin outcomes
- robust standalone bathtub-filter market-size evidence

## Access / retrieval note
Some full-text materials were only available at abstract, snippet, or index level during this pass. Attempts to use wrapper/access tools (e.g. 12ft, Unpaywall-style routes, paywall-removal mirrors) did not yield dependable full-text retrieval in this session, so current entries rely on visible summaries unless otherwise noted.

## Sources
- [Source Summary — Bathtub Filter Academic and Institutional Evidence (2026-04-11)](../source-summaries/bathtub-filter-academic-and-institutional-evidence-2026-04-11.md)
- [Source Summary — Bathtub Filter Academic and Institutional Evidence Deepening (2026-04-11)](../source-summaries/bathtub-filter-academic-and-institutional-evidence-deepening-2026-04-11.md)
- [Source Summary — Bathtub Filter Market Research Signals (2026-04-11)](../source-summaries/bathtub-filter-market-research-signals-2026-04-11.md)
- [Source Summary — Bathtub Filter Market Research and Industry Explanation Deepening (2026-04-11)](../source-summaries/bathtub-filter-market-research-and-industry-explanation-deepening-2026-04-11.md)
- [Source Summary — Bathtub Filter Web Content Fetch Attempts (2026-04-11)](../source-summaries/bathtub-filter-web-content-fetch-attempts-2026-04-11.md)
- [Source Summary — Bathtub Filter Evidence Access Audit (2026-04-11)](../source-summaries/bathtub-filter-evidence-access-audit-2026-04-11.md)
