...
| 7 | KDF-55 产品手册 | Onlyzone | 未注明 | SKU + 参数 + 手册内 200g@7 L/min 70–80% 游离氯去除测试 | 支撑 "KDF 在此流速下游离氯去除率约 70–80%" 量级 | 手册非检测报告 |
| 8 | 长效碱性球手册 | Onlyzone | 未注明 | pH 提升 + 寿命 10,000 L+ | 可作为 optional loading-cavity 备选 | **与 Version A "不以 TDS 为目标" 定位冲突**——碱性球会提升 pH 和 TDS |

### 2.5）CaSO3 材料外观初筛（来料质检线索）

这部分不是认证证据，也不能替代自由氯去除测试；它只用于样品来料时的材料初筛和供应商沟通。

较好的 CaSO3 球 / 颗粒通常应表现为：
- 颜色均匀
- 颗粒规整
- 粉尘少
- 不易碎
...
使用边界：
- 外观良好只能支持“来料一致性较好”的判断，不能直接支持“游离氯去除率更高”。
- 外观异常应触发留样、拍照、称重掉粉、浊度、破碎率和自由氯前后对比测试。
- 对外文案只能讲“透明滤盒可见滤材”，不能把“看起来好”写成认证或性能结论。
